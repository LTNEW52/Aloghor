    ...
    <>
      <div className="w-full flex">
        ...
        <div className="block textGolden w-fit h-fit p-5 text-4xl font-extrabold ml-25 mt-[15px] textShadow">
          <nav>
            <Link to="/" className="mr-10  hover:text-[hsl(37,100%,78%)] hover:underline">
              সূচনা
            </Link>
            <Link to="/Book" className="mr-10 hover:text-[hsl(37,100%,78%)] hover:underline">
              বই সমগ্র
            </Link>
            <Link to="/Discussion" className="mr-10 hover:text-[hsl(37,100%,78%)] hover:underline">
              আলোচনার আসর
            </Link>
            <Link to="/Events" className="mr-10  hover:text-[hsl(37,100%,78%)] hover:underline">
              সাহিত্য আয়োজন
            </Link>
          </nav>
        </div>
        ...
        <div className="relative mt-8 mr-10">
          <input
            ...
            className="bg-white rounded-full w-70 h-10 pl-5 text-[20px] placeholder:text-[#6e6a6a]"
          />
          <button className="h-9 w-9 absolute right-0.5 top-0.5 bg-black rounded-full border-none cursor-pointer hover:bg-[hsl(0,0%,20%)]">
            <img src={searchIcon} alt="Search Icon" className="w-5.5 m-auto animate-pulse" />
          </button>
        </div>

        <div
          ref={ref}
          className="textGolden w-13 h-13 mt-6 rounded-full backgroundGreen cursor-pointer"
        >
          <p
            className="text-[33px] text-center hover:bg-[hsl(92,30%,27%)] hover:rounded-full"
            onClick={() => {
          ...
          <div
            className={`z-20 h-fit mt-5 p-10 text-2xl backgroundGreen absolute right-3 rounded-[15px] border-none boxShadowleft flex flex-col ${showInnerDiv ? "" : "hidden"}`}
          >
            <Link to="/User-Authentication" className="mb-5 hover:text-[hsl(37,100%,78%)]">
              Login Or SignUp
            </Link>
            <Link to="" className="mb-5 hover:text-[hsl(37,100%,78%)]">
              Settings
            </Link>
            <Link to="" className="hover:text-[hsl(37,100%,78%)]">
              Logout
            </Link>
          </div>
        </div>
